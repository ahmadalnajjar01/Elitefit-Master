...
const Favorite = () => {
  ...
  // Updated toast functions to match Checkout component style
  ...
  const handleRemoveFromFavorite = (id, name) => {
    ...
    showToast("info", `${name} removed from favorites`, { icon: "🗑️" });
  };
  ...
  // Handle product detail view with toast notification
  const handleViewDetails = (item) => {
    showToast("info", `Viewing ${item.name} details...`);
    navigate(`/product/${item.id}`, { state: { item } });
  };

  return (
    <div className="font-sans bg-white min-h-screen p-6">
      {/* Toast Container with the same styling as Checkout component */}
      <Toaster
        position="top-right"
        toastOptions={{
          style: {
